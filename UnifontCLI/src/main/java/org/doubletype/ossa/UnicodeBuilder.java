...
public class UnicodeBuilder {
	...
	public static Long askUnicode() {
		...
		String s = JOptionPane.showInputDialog(
		        engine.getBundle().getString("msgUnicode"));
		
		return build(s, Engine.SEARCH_UNICODE);
	}
    ...
    private static Long buildByExample(String a_value) {
        if (a_value.length() > 1) {
            JOptionPane.showMessageDialog(null, "The example must be a letter.");
    ...
    private static Long buildByJisCode(String a_value) { 
		...
		try {
			...
			if (retval == null) {
			    JOptionPane.showMessageDialog(null, "Bad JIS Code.");
			} // if
