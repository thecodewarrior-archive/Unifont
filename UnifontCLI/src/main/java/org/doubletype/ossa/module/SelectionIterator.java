...
import org.doubletype.ossa.action.GlyphAction;
...
public class SelectionIterator extends GlyphIterator implements Iterator {
    ...
    protected void buildList() {
        ...
        for (int i = m_list.size() - 1; i >= 0; i--) {
		    Object object = m_list.get(i);
		    
		    // ignore non-point objects while hitting points and hints
		    if (GlyphAction.isPointVisible()) {
		        if (!(object instanceof EContourPoint)
		            && !(object instanceof EHint)
		            && !(object instanceof EControlPoint)) {
		            continue;
		        } // if
		    } // if
