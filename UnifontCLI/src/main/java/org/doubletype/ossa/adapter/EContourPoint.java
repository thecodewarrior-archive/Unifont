...
public class EContourPoint extends XContourPoint implements GlyphObject, EPoint {	
	...
	public void display(Graphics2D g, AffineTransform a_trans) {		
	    ...
	    int ppem = EObject.getDisplay().getPpem();	
	...
	private void displayNumber(Graphics2D g, AffineTransform a_trans) {
		double x = getX();
		double y = getY();
		
	    AffineTransform oldTrans = g.getTransform();
		g.setFont(EObject.getDisplay().getNumberFont());		
		g.setColor(GlyphColor.NUMBER);

		g.transform(a_trans);
		g.translate(x + 30, y);
		g.scale(1, -1);
		g.drawString(Integer.toString(m_number), 0, 0);
	...
	public boolean hasHintForCurrentPpem() {
	    return hasHintForPpem(EObject.getDisplay().getPpem());
	}
	...
	public EHint getCurrentHint() {
	    return getHintForPpem(EObject.getDisplay().getPpem());
	}
